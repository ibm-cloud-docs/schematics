...
{{site.data.keyword.bpshort}} concurrently supports multiple images that each package a specific version of the {{site.data.keyword.cloud_notm}} Provider plug-in, other Terraform providers, such as Helm or Kubernetes, and the Ansible executable. Each image is built, tested, and verified by {{site.data.keyword.cloud_notm}}. Included Terraform provider versions are tested and packaged for a specific Terraform CLI version. 
...
To use any of the pre-defined [{{site.data.keyword.bpshort}} images](#schematics-image-ov), you must explicitely declare the version of the {{site.data.keyword.cloud_notm}} Provider plug-in in your Terraform template that includes the provider versions that you want. 

If no {{site.data.keyword.cloud_notm}} Provider plug-in version is declared in your Terraform template, the latest version is automatically used in {{site.data.keyword.bpshort}}. 
{: note}
