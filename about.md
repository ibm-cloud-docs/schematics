...
|Control access to cloud environments and configurations|{{site.data.keyword.bpshort}} is fully integrated with {{site.data.keyword.cloud_notm}} Identity and Access Management (IAM) so that you can use service access roles to control who can access and collaborate on your workspaces and actions, or roll out changes. You can invite {{site.data.keyword.cloud_notm}} users to your account and leverage IAM access groups to streamline the access assignment process in your organization. For more information about how your workloads are isolated from other tenants in {{site.data.keyword.cloud_notm}}, see [Workload isolation](/docs/schematics?topic=schematics-compute-isolation#workload-isolation). |
