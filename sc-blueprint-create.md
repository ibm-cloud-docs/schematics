...
Where the user has the required IAM access to create resources groups, use the parameters from the 'create resource group' line. Where a user only has access to an existing group, use the parameters from the 'resuse resource group' line.  
...
The next step in deploying the cloud resources that are defined by the blueprint is to [Apply](/docs/schematics?topic=schematics-apply-blueprint) the blueprint config. 
