...
|----------------|-----------|-------------|
|Create workspace|<img src="images/createworkspace.png" alt="Create workspace state"  width="100" style="width: 100px; border-style: none"/>|The workspace is created without a reference to GitHub or GitLab to the draft state. From the draft state you can connect to the template in your source repository. From connecting state, the template is processed successfully to reach Inactive state (Final state) or template process may fail and reach failed state. From inactive state you can maintain atleast one resource in the state file to be in active state and then, can destory all the resources and to make your workspace in an inactive state.|
|Delete workspace|<img src="images/deleteworkspace.png" alt="Delete workspace state"  width="100" style="width: 100px; border-style: none"/>|The delete workspace can be performed when you workspace is in Inactive, Active or Failed state. From these state, you can connect to the template in your source repository and remove the workspace to reach the final state. From the inactive state, if you delete at least one resource, the plan and apply will be executed to destroy the resource from the active state.|
