...
IBM Cloud Schematics supports 50 API requests per minute, per host, and per customer. The region can be us-east, us-south, eu-gb, or eu-de region. You need to wait before calling the command again. Ensure the location and the url endpoint are pointing to the same region when you list workspaces.
{: shortdesc} 
...
1. List the workspaces in your IBM Cloud account and optionally, show the details for your workspace. For more configuration options when listing the workspace, see the [`ibmcloud schematics workspace list` command](/docs/schematics?topic=schematics-schematics-cli-reference#schematics-workspace-list).
...
| Flag | Required / Optional |Description |
    | --- |  --- |
    | `--limit` value, -l value   | Maximum number of workspaces to list. Ignored if a negative number is set. Maximum: 200. (default: -1)
    | `--offset` value, -m value  | Offset in list. Ignored if a negative number is set (default: -1)
    | `--json`, -j                | [Deprecated] Prints the output as JSON. Use --output JSON instead
    | `--output` value, -o value  |Specify output format, only 'JSON' is supported
    | `--region` value, -r value  | Specify region, i.e eu, us, eu-gb, eu-de, us-south, us-east
