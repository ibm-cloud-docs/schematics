...
3. Optional: Review the variables that you set for your workspace. The values of your variables are used every time that you reference the variable in your Terraform template. Make sure that your variables use the correct values.  
4. Click **Generate plan** to create a Terraform execution plan. This action equals the `terraform plan` command. After you click the button, the workspace **Activity** page opens. 
5. Click **View log** to review the log files of your execution plan. The execution plan includes a summary of {{site.data.keyword.cloud_notm}} resources that must be created, modified, or deleted to achieve the state that you described in your Terraform template. If you have syntax errors in your Terraform configuration files, you can review the error message in the log file. 
...
2. Click **Pull latest** to get the latest version of your Terraform template from the linked GitHub source repository. 
3. If you added, or removed variables in your Terraform configuration files, or if you want to change the variable values that you set when you created the workspace, enter or change the variable values. 
4. Click **Generate plan** to create a Terraform execution plan. The workspace **Activity** page opens. 
...
6. From the workspace details page, click **Apply plan** to apply the new Terraform template version. The workspace **Activity** page opens. Depending on the changes that you made, it might take a few minutes or up to a few hours for the template to be applied.   
...
6. To review events that Schematics sent to {{site.data.keyword.cloudaccesstrailshort}}, see [](/docs/schematics?topic=schematics-at-events). 
...
6. Click **Generate plan** to create a Terraform execution plan. The workspace **Activity** page opens. 
7. Click **View log** to review the log files of your execution plan. The log files provide a summary of all the resources that {{site.data.keyword.cloud_notm}} Schematics is about to remove. 
8. From the workspace details page, click **Apply plan** to remove the {{site.data.keyword.cloud_notm}} resources from your account. The workspace **Activity** page opens. 
...
**To remove your resources from the Schematics console**: 
