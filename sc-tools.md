...
No additional software or tools can be installed in the {{site.data.keyword.bpshort}} runtime. Any attempt to install a software in the `schematics-runtime-job` pod, is considered a violation and can cause vulnerability.
{: note}
...
The {{site.data.keyword.bpshort}} Agents uses the following Terraform versions in the `Terraform-runtime-agent-job` image:
•	Terraform 1.0.x, 1.1.x
...
The {{site.data.keyword.bpshort}} Actions uses the following Terraform versions in the `Ansible-runtime-job` image:
•	Ansible v2.9.x
