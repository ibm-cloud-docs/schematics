...
- For `API` you can see [create or update the values](/apidocs/schematics#createworkspace) in the field **template_data** &gt;  **variablestore**. The `value` field is the `HCL` format value as provided in the `.tfvars` file. It is always a JSON string irrespective of the type of the variable. The value must contain escape strng for the variablestore.
